    ...
    if (user && user.password == password) {
      if (user.isActive) {
        ...
        const refreshToken = withRefresh
          ? jwt.sign(refreshData, process.env.REFRESH_KEY, { expiresIn: "3d" })    //--> üstteki eklemeye istinaden varsa al yoksa null
          : null;
        ...
        // throw new Error('This account is not active.')  //----------->>>>> artık bu bir MW olmadığı için fonk içerisinde res. çalışmayacak onun yerine return yapıorz
        return {
          error: true,
